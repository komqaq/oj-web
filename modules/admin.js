...
async function cryptPwd(password, salt) {
  // 密码“加盐”
...
async function registeUser(username, password, email) {
	var md5 = crypto.createHash('md5');
	password = md5.update(password + "syzoj2_xxx").digest('hex');
...
app.post('/admin/register', async (req, res) => {
  try {
    ...
    if(req.body.username != "") {
      try {
        registeUser(req.body.username, req.body.password, req.body.email)

        var messages = new Array()
        messages[0] = "账号: " + req.body.username + " 密码: " + req.body.password + " 邮箱:" + req.body.email
        res.render('success', {
          messages: messages,
          title: "注册成功"
        })
    ...
    if(req.body.prefix != "") {
      try {
        ...
        var start = parseInt(req.body.startnum)
        var end = parseInt(req.body.endnum)

        for (var i=start; i<=end; i++) {
          password = getRandomSalt()
          for(let j = 0; j<10; j++) password = password + getRandomSalt()
          password = await cryptPwd(password, getRandomSalt)
          var username = req.body.prefix + i
          var email = "none@none.com"
          password = password.slice(0, 16);
          registeUser(username, password, email)
          messages[messages.length] = "账号: " + username + " 密码: " + password + " 邮箱:" + email
        }
      ...
      throw new ErrorMessage("批量注册失败，请重试。")
      }
    }

    //res.redirect(syzoj.utils.makeUrl(['admin', 'register']));
  } catch (e) {
    syzoj.log(e);
    res.render('error', {
      err: e
