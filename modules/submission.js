...
app.get('/submission/:id', async (req, res) => {
  try {
    ...
    if (judge.problem.type !== 'submit-answer') {
      ...
      // modified
      var isac = await judge.problem.getJudgeState(res.locals.user, true);
      if(res.locals.user && (judge.user.username == res.locals.user.username || await judge.problem.isAllowedEditBy(res.locals.user)) || (isac && isac.status == 'Accepted')) {
      	judge.code = await syzoj.utils.highlight(judge.code, syzoj.config.languages[judge.language].highlight);
  		}
      else {
      	judge.code = "你还没有 AC 这道题，不能查看他人代码。"
      }
    }
