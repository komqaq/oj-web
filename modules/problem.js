...
app.post('/problem/:id/submit', app.multer.fields([{ name: 'answer', maxCount: 1 }]), async (req, res) => {
  try {
    ...
    if (contest_id) {
      ...
      if (((!contest.isRunning()) && (!contest.isEnded())) && (!await contest.isSupervisior(curUser))) throw new ErrorMessage('比赛未开始或已结束。');
...
app.get('/problem/:id/testdata/download/:filename?', async (req, res) => {
  try {
    ...
    if (req.params.id == 'lydsy') {
      ...
      let filename = path.join(syzoj.utils.resolvePath(syzoj.config.upload_dir, 'testdata','lydsy'), req.params.filename);
      ...
      if (!problem) throw new ErrorMessage('无此题目。');
    // modified
      if (!await problem.isAllowedEditBy(res.locals.user) && (req.params.filename.indexOf(".jpg")==-1) && (req.params.filename.indexOf(".jpeg")==-1) && (req.params.filename.indexOf(".png")==-1)) throw new ErrorMessage('您没有权限进行此操作。');
...
app.get('/problem/:id/download/additional_file', async (req, res) => {
  try {
    ...
    if (contest_id) {
      ...
      if (!contest) throw new ErrorMessage('无此比赛。');
      if (!contest.isRunning()&&!contest.isEnded()) throw new ErrorMessage('比赛未开始或已结束。');
      let problems_id = await contest.getProblems();
      if (!problems_id.includes(id)) throw new ErrorMessage('无此题目。');
    } else {
      if (!await problem.isAllowedUseBy(res.locals.user)) throw new ErrorMessage('您没有权限进行此操作。');
