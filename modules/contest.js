...
let ContestPrivate = syzoj.model('contest_private');
let ContestPlayerAllow = syzoj.model('contest_player_allow');
...
app.get('/contest/:id/edit', async (req, res) => {
  try {
    ...
    let contestprivate = await ContestPrivate.fromID(contest_id);
    if (!contest) {
      contest = await Contest.create();
      contestprivate = await ContestPrivate.create();
      contest.id = 0;
    } else {
      await contest.loadRelationships();
      await contest.loadRelationships2();
    }
...
app.post('/contest/:id/edit', async (req, res) => {
  try {
    ...
    if (!contest) {
      ...
      if (!['noi', 'ioi', 'acm', 'pku'].includes(req.body.type)) throw new ErrorMessage('无效的赛制。');
    ...
    contestprivate.is_private = req.body.is_private === 'on';
    if(syzoj.utils.isValidUsername(req.body.contest_password)) contestprivate.password = req.body.contest_password;
    await contestprivate.save();
...
app.get('/contest/:id', async (req, res) => {
  try {
    ...
    let playerallow = await ContestPlayerAllow.findInContest({
      contest_id: contest_id,
      user_id: res.locals.user.id
    });
    let contestprivate = await ContestPrivate.fromID(contest_id);
    if (!playerallow && !res.locals.user.is_admin && contestprivate.is_private) {
        res.render('contest_passwd', {
        contest: contest
      });
    } else {
      ...
      if (res.locals.user) {
        player = await ContestPlayer2.findInContest({
      ...
      if (player) {
        for (let problem of problems) {
          ...
          } else if (contest.type === 'ioi' || contest.type === 'pku') {
            if (player.score_details[problem.problem.id]) {
              let judge_state = await JudgeState.fromID(player.score_details[problem.problem.id].judge_id);
              problem.status = judge_state.status;
              problem.judge_id = player.score_details[problem.problem.id].judge_id;
      ...
      if ((!contest.hide_statistics) || (contest.ended) || (isSupervisior)) {
        ...
        for (let problem of problems) {
          ...
          if (contest.type === 'ioi' || contest.type === 'noi' || contest.type === 'pku') {
          ...
          for (let player of players) {
            if (player.score_details[problem.problem.id]) {
              ...
              if ((contest.type === 'acm' && player.score_details[problem.problem.id].accepted) || ((contest.type === 'noi' || contest.type === 'ioi' || contest.type === 'pku') && player.score_details[problem.problem.id].score === 100)) {
                problem.statistics.accepted++;
              }

              if ((contest.type === 'noi' || contest.type === 'ioi' || contest.type === 'pku') && player.score_details[problem.problem.id].score > 0) {
...
app.post('/contest/:id', async (req, res) => {
  try {
    ...
    if (!syzoj.utils.isValidUsername(req.body.password))
      throw new ErrorMessage('密码错误');
    if (contestprivate.is_private && contestprivate.password !== req.body.password)
      throw new ErrorMessage('密码错误');

    let contestplayerallow = await ContestPlayerAllow.create({
      contest_id: contest_id,
      user_id: res.locals.user.id
    });
    await contestplayerallow.save();
...
app.get('/contest/:id/ranklist1', async (req, res) => {
  try {
    ...
    if (!contest) throw new ErrorMessage('无此比赛。');
    if (!contest.isRunning() && !contest.isEnded()) throw new ErrorMessage('比赛还没有开始，请耐心等待哦 (´∀ `)');
    if (!contest.is_public && (!res.locals.user || !res.locals.user.is_admin)) throw new ErrorMessage('比赛未公开，请耐心等待 (´∀ `)');
    ...
    let ranklist = await players_id.mapAsync(async player_id => {
      ...
      if (contest.type === 'noi' || contest.type === 'ioi' || contest.type === 'pku') {
        player.score = 0;
      }
...
app.get('/contest/:id/ranklist2', async (req, res) => {
  try {
    ...
    let ranklist2 = await players_id.mapAsync(async player_id => {
      let player = await ContestPlayer2.fromID(player_id);
