...
class Contest extends Model {
  ...
  allowedSeeingOthers() {
    if (this.type === 'acm' || this.type === 'pku') return true;
    else return false;
  }

  allowedSeeingScore() { // If not, then the user can only see status
    if (this.type === 'ioi' || this.type === 'pku') return true;
    else return false;
  }

  allowedSeeingUsage() { // If not, then the user cannot see time and memory
    if (this.type === 'pku') return true;
    else return false;
  }

  allowedSeeingCode() { // If not, then the user cannot see the code details (such as the size)
    if (this.type === 'pku') return true;
    else return false;
  }

  allowedSeeingResult() { // If not, then the user can only see compile progress
    if (this.type === 'ioi' || this.type === 'acm' || this.type === 'pku') return true;
    else return false;
  }
